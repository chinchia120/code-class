function [] = plotpatchslip3D_vectors(pm,slip,nve)
%
...
for k=2:nhe
	index1 = nve*(k-1);
	index2 = nve*(k-2);
	x(index1+1:index1+nve, :) = [x(index2+1:index2+nve,2), x(index2+1:index2+nve,2) + pm(index1+1,1)*ones(nve,1)];
end
...
%{
fid=fopen('gmt_faultplane_x.gmt','w');
fprintf(fid,'%10.4f %10.4f %10.4f %10.4f\r\n',xvert);
fclose(fid);
fid=fopen('gmt_faultplane_y.gmt','w');
fprintf(fid,'%10.4f %10.4f %10.4f %10.4f\r\n',yvert);
fclose(fid);
fid=fopen('gmt_faultplane_z.gmt','w');
fprintf(fid,'%10.4f %10.4f %10.4f %10.4f\r\n',zvert);
fclose(fid);
fid=fopen('gmt_faultslip.gmt','w');
fprintf(fid,'%10.4f %10.4f %10.4f\r\n',[ss;ds;ns]);
fclose(fid);
%}

% 
patch(xvert,yvert,zvert,ns), axis('equal')
colormap(flipud(hot))
%colormap(jet)
colorbar
hold on
...
quiver3(centersx, centersy, centersz+0.01, slipvec(:,1), slipvec(:,2), slipvec(:,3), 'k')

axis equal
view([45 45])

%{
fid=fopen('gmt_faultslip_vector.gmt','w');
