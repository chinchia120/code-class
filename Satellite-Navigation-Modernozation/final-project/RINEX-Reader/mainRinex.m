...
%% =============== Select nav File =============== %%
[navfname, navpname] = uigetfile({'*.nav'}, 'Please select your nav file', obspname);
filePath_nav = [navpname navfname];
[outputEphemeris] = readRinexNav(filePath_nav);

%% =============== Save Observation Data (obs) into rcvr.dat =============== %%
fid1 = fopen([obspname extractBefore(obsfname, '.obs'), '_rcvr.dat'], 'w+');
[m,n]=size(obs);
for i=1:1:m
    for j=1:1:n
        if j==n
...
fid2 = fopen([navpname extractBefore(obsfname, '.obs'), '_eph.dat'], 'w+');
